 ...
 * Enable camera to capture images and save them on current directory as .jpg.A
 * Control brightness using 'u' to increase and 'd' to decrease.
 */
...
static void *thread_brightness_control (void* args) {
    ...
    if (0 != ioctl(fd, VIDIOC_QUERYCTRL, &qctrl)) {
        perror("Unable to query brightness");
        return NULL;
    }
    ...
    if (0 != ioctl(fd, VIDIOC_G_CTRL, &ctrl)) {
        perror("Unable to get brightness");
        return NULL;
    }
    printf(", current: %d\n", ctrl.value);
...
int main (int argc, char *argv[]) {
    ...
    /* Create a thread to control brightness. */
    pthread_t thread_brightness;
    pthread_create(&thread_brightness, NULL, thread_brightness_control, (void *)fd);
